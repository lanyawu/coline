...
#include <miniupnp/minisoap.h>
#include <miniupnp/minixml.h>
...
#ifdef WIN32
	sudp = (int)socket(PF_INET, SOCK_DGRAM, IPPROTO_UDP);
#else
                 ...
                 const char * descURL)
{
	...
	n2 = n1; n3 = n1;
	n1 += (int)strlen(data->scpdurl);
	n2 += (int)strlen(data->controlurl);
	n3 += (int)strlen(data->controlurl_CIF);
