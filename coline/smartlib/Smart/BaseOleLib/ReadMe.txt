========================================================================
    活动模板库：BaseOleLib 项目概述
========================================================================

应用程序向导已为您创建了此 BaseOleLib 项目，您可以在此基础之上
编写自己的动态链接库 (DLL)。
...
BaseOleLib.idl
    ...
    和 coclass 的 IDL 定义。
    此文件将由 MIDL 编译器处理以生成：
        C++ 接口定义和 GUID 声明                 (BaseOleLib.h)
        GUID 定义                                (BaseOleLib_i.c)
        一个类型库                               (BaseOleLib.tlb)
        封送代码                                 (BaseOleLib_p.c 和 dlldata.c)

BaseOleLib.h
    此文件包含在 BaseOleLib.idl 中定义的项的 C++ 接口定义
...
StdAfx.h，StdAfx.cpp
    这些文件用于生成名为 BaseOleLib.pch 的预编译头 (PCH) 文件
...
BaseOleLibps.vcproj
	...
	dlldata.c、BaseOleLib_i.c 和 BaseOleLib_p.c，它们。
