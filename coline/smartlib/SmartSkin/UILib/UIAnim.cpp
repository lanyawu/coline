...
CAnimJobUI::CAnimJobUI(UITYPE_ANIM AnimType, DWORD dwStartTick, DWORD dwDuration, COLORREF clrBack, 
...
bool CAnimationSpooler::Init(HWND hWnd)
{
	...
	// Is window topmost?
	HWND hWndFocus = hWnd;
	while(::GetParent(hWndFocus) != NULL)
...
void CAnimationSpooler::Term()
{
	...
	int i;
	for (i = 0; i < m_aJobs.GetSize(); i++)
		delete static_cast<CAnimJobUI*>(m_aJobs[i]);
	m_aJobs.Empty();
...
bool CAnimationSpooler::PrepareJob_Flat(CAnimJobUI* pJob)
{
	...
	for (int x = rc.left; x < rc.right; x += iTexSize) 
	{
		for (int y = rc.top; y < rc.bottom; y += iTexSize)
		{
			...
			Hr = m_p3DDevice->CreateVertexBuffer(4 * sizeof(CUSTOMVERTEX), D3DUSAGE_WRITEONLY, D3DFVF_CUSTOMVERTEX, 
			                  D3DPOOL_DEFAULT, &pVertex, NULL);
			if (FAILED(Hr))
				return false;
			...
			Hr = m_p3DDevice->CreateTexture(iTexSize, iTexSize, 1, D3DUSAGE_RENDERTARGET, D3DFMT_A8R8G8B8,
			                       D3DPOOL_DEFAULT, &pTex2, NULL);
			if (FAILED(Hr))
				return false;
...
bool CAnimationSpooler::RenderJob_Flat(const CAnimJobUI* pJob, LPDIRECT3DSURFACE9 /*pSurface*/, DWORD dwTick)
{
	...
	FLOAT scale1 = 0.0;
	if (pJob->data.plot.iInterpolate == CAnimJobUI::INTERPOLATE_LINEAR)
		scale1 = (FLOAT) LinearInterpolate(0.0, 1.0, mu);
	if (pJob->data.plot.iInterpolate == CAnimJobUI::INTERPOLATE_COS) 
