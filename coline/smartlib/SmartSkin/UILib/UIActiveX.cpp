...
{
   m_pOwner = pOwner;
   UINT uStyle = WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_THICKFRAME;
...
void CActiveXUI::ParentChangeVisible(bool bVisible)
{
	/*if (m_pControl && !m_pControl->m_bWindowless)
	{
		if (!bVisible)
		{
			m_pControl->m_pWindow->ShowWindow(FALSE);
		} else
		{
			if (IsVisible())
				m_pControl->m_pWindow->ShowWindow(TRUE, FALSE);
		} //end else if (
	} //end if (m_pControl &&  */
	SetVisible(bVisible);
}
...
void CActiveXUI::SetVisible(bool bVisible)
{
	...
	if (m_pControl && !m_pControl->m_bWindowless)
	{
		m_pControl->m_pWindow->ShowWindow(bVisible);
	}
}
...
//class  CInternetExplorerUI
CInternetExplorerUI::CInternetExplorerUI():
                     CActiveXUI(),
					 m_pWebBrowser(NULL)
{
...
void CInternetExplorerUI::Navigate2(const TCHAR *szwURL)
{
	m_sLatestUrl = szwURL;
	if (m_pWebBrowser)
	{
		VARIANT vt;
		vt.vt = VT_BSTR;
		vt.bstrVal = SysAllocString(szwURL);
		m_pWebBrowser->Navigate2(&vt, NULL, NULL, NULL, NULL);
		SysFreeString(vt.bstrVal); //释放内存
	}
}

void CInternetExplorerUI::RefreshDocument()
{
	//
}

void CInternetExplorerUI::Navigate(const char *szURL)
{
	if (szURL)
	{
		int nLen = ::strlen(szURL) * 2;
		TCHAR *szwURL = new TCHAR[nLen];
		memset(szwURL, 0, nLen * sizeof(TCHAR));
		CStringConversion::StringToWideChar(szURL, szwURL, nLen - 1); 
	    Navigate2(szwURL);
		delete []szwURL;
	}
}

void CInternetExplorerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	{
		Navigate2(pstrValue);
	} else if (_tcsicmp(pstrName, _T("document")) == 0)
	{
		m_strDocument = pstrValue;
		RefreshDocument();
	} else
	{
...
bool CInternetExplorerUI::DelayedControlCreation()
{
	bool b = CActiveXUI::DelayedControlCreation();
	if (b && (!m_pWebBrowser) && m_pUnk)
	{
		m_pUnk->QueryInterface(IID_IWebBrowser2, (LPVOID *)&m_pWebBrowser);
		if (!m_sLatestUrl.IsEmpty())
		{
			VARIANT vt;
			vt.vt = VT_BSTR;
			vt.bstrVal = SysAllocString(m_sLatestUrl.GetData());
			m_pWebBrowser->Navigate2(&vt, NULL, NULL, NULL, NULL); 
			SysFreeString(vt.bstrVal); //释放内存
		}
	}
