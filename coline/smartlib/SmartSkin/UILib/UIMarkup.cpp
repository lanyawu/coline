...
void CMarkup::Release()
{
   ...
   if( m_pElements != NULL ) delete [] m_pElements;//free(m_pElements);
   m_pstrXML = NULL;
   m_pElements = NULL;
   m_nElements = 0;
}
...
CMarkup::XMLELEMENT* CMarkup::_ReserveElement()
{
   ...
   if( m_nElements >= m_nReservedElements ) {
	  ...
	  if( m_pElements )
	     ...
	     //m_pElements = static_cast<XMLELEMENT*>(malloc(m_nReservedElements * sizeof(XMLELEMENT)));
	  try{
	     m_pElements = new XMLELEMENT[1];
	  }
	  catch( ... ){
		  return NULL;
	  }
...
{   
   ...
   *pstrText++ = '\0';
   ...
   while( *pstrText != '\0' && *pstrText != '>' && *pstrText != '/' ) {
      ...
      *pstrText++ = '\0';
      if( *pstrText++ != '\"' ) return _Failed(_T("Expected attribute value"), pstrText);
      LPTSTR pstrDest = pstrText;
      if( !_ParseData(pstrText, pstrDest, '\"') ) return false;
